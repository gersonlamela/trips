...
export default function TripsSearch() {
	...
	return (
		<div className="container mx-auto flex flex-col items-center p-5">
			...
			<h2 className="text-grayPrimary font-medium  mb-5">{trips.length > 0 ? 'Listamos as melhores viagens para ti!' : 'Não encontramos nenhuma viagem =('}</h2>
			<div className="flex flex-col gap-4">{trips?.map((trip) => <TripItem key={trip.id} trip={trip} />)}</div>
		</div>
	);
}
