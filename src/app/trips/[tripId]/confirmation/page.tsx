...
export default function TripConfirmation({params}: {params: {tripId: string}}) {
	...
	const handleBuyClick = async () => {
		const res = await fetch('http:///api/payment', {
			method: 'POST',
			body: Buffer.from(
				JSON.stringify({
					tripId: params.tripId,
	...
	return (
		<div className="container mx-auto p-5">
			...
			{/*Card*/}
			<div className="flex flex-col p-5 mt-5 border-grayLighter border-solid shadow-lg rounded-lg">
			...
			<div className="flex flex-col mt-5 text-primaryDarker">
				...
				<div className="flex items-center gap-1">
					<p>{format(startDate, "dd 'de' MMMM", {locale: pt})}</p>
					{' - '}
					<p>{format(endDate, "dd 'de' MMMM", {locale: pt})}</p>
				</div>
